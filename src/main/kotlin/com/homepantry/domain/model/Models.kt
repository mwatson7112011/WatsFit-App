...
enum class StorageLocation(val displayName: String) {
    ...
    companion object {
        fun fromString(value: String): StorageLocation =
            entries.firstOrNull { it.name.equals(value, ignoreCase = true) } ?: PANTRY
    }
}
...
enum class TextureCategory(val displayName: String) {
    ...
    companion object {
        fun fromString(value: String): TextureCategory =
            entries.firstOrNull { it.name.equals(value, ignoreCase = true) } ?: TENDER
    }
}
...
enum class Cuisine(val displayName: String) {
    ...
    companion object {
        fun fromString(value: String): Cuisine =
            entries.firstOrNull { it.name.equals(value, ignoreCase = true) } ?: MEDITERRANEAN
    }
}
...
data class Recipe(
    ...
    val totalTimeMin: Int?
        get() {
            val prep = prepTimeMin ?: return cookTimeMin
            val cook = cookTimeMin ?: return prepTimeMin
            return prep + cook
        }
}
...
data class WorkoutRoutine(
    ...
    val title: String,
    val description: String,
    val category: String, // "push", "pull", "legs", "core", "fullbody", "flexibility", "cardio"
    val difficulty: Difficulty = Difficulty.BEGINNER,
    val durationMin: Int = 30,
    val exercises: List<WorkoutExerciseEntry> = emptyList(),
    val dayOfProgram: Int? = null,
    val tags: List<String> = emptyList()
)
...
enum class HeartRateContext(val displayName: String) {
    ...
    companion object {
        fun fromString(value: String): HeartRateContext =
            entries.firstOrNull { it.name.equals(value, ignoreCase = true) } ?: RESTING
    }
}
...
data class SeedWorkoutRoutine(
    ...
    val difficulty: String = "beginner",
    val durationMin: Int = 30,
    val exercises: List<WorkoutExerciseEntry>,
    val dayOfProgram: Int? = null,
    val tags: List<String> = emptyList()
)

@Serializable
data class SeedWorkoutFile(
    val exercises: List<SeedExercise>,
    val routines: List<SeedWorkoutRoutine>
)
